...
CREATE TABLE AEROPUERTO(
    ...
    CONSTRAINT FK_AEROPUERTO_PAIS FOREIGN KEY (id_pais) REFERENCES PAIS(id_pais)
    ON DELETE CASCADE
);
...
CREATE TABLE VUELO(
    ...
    CONSTRAINT FK_VUELO_AEROLINEA FOREIGN KEY (id_aerolinea_vuel) REFERENCES AEROLINEA(id_aerolinea),
    CONSTRAINT FK_VUELO_AEROPUERTO_ORIGEN FOREIGN KEY (id_aeropuerto_origen) REFERENCES AEROPUERTO(id_aeropuerto),
    CONSTRAINT FK_VUELO_AEROPUERTO_DESTINO FOREIGN KEY (id_aeropuerto_destino) REFERENCES AEROPUERTO(id_aeropuerto) 
    ON DELETE CASCADE 
);
...
CREATE TABLE COMPRA(
    ...
    CONSTRAINT FK_COMPRA_PASAJERO FOREIGN KEY (id_pasajero_comp) REFERENCES PASAJERO(documento_identidad),
    CONSTRAINT FK_COMPRA_VUELO FOREIGN KEY (id_vuelo_comp) REFERENCES VUELO(id_vuelo)
    ON DELETE CASCADE
);
...
CREATE TABLE RESERVA(
    ...
    CONSTRAINT FK_RESERVA_PASAJERO FOREIGN KEY (id_pasajero_res) REFERENCES PASAJERO(documento_identidad),
    CONSTRAINT FK_RESERVA_VUELO FOREIGN KEY (id_vuelo_res) REFERENCES VUELO(id_vuelo)
    ON DELETE CASCADE
);
...
CREATE TABLE EQUIPAJE(
    ...
    CONSTRAINT FK_EQUIPAJE_PASAJERO FOREIGN KEY (id_pasajero_eq) REFERENCES PASAJERO(documento_identidad),
    CONSTRAINT FK_EQUIPAJE_VUELO FOREIGN KEY (id_vuelo_eq) REFERENCES VUELO(id_vuelo)
    ON DELETE CASCADE
);
...
CREATE TABLE SERVICIOS(
    ...
    CONSTRAINT FK_SERVICIOS_PASAJERO FOREIGN KEY (id_pasajero_ser) REFERENCES PASAJERO(documento_identidad)
    ON DELETE CASCADE
);
...
CREATE TABLE ASISTENCIA(
    ...
    CONSTRAINT FK_ASISTENCIA_PASAJERO FOREIGN KEY (id_pasajero_asi) REFERENCES PASAJERO(documento_identidad),
...
FOR EACH ROW
BEGIN
    IF EXISTS (SELECT 1 FROM RESERVA WHERE id_asiento = :NEW.id_asiento AND estado = 'Confirmado') THEN
